...
module.exports = async (req, res) => {
  ...
  if (!isSingle && !isBulk) {
    return res
      .status(400)
      .json({ error: "Provide url (single) or urls (array) in request body" });
  }

  // Vercel serverless max = 60s, limit bulk to 5 URLs to stay safe
  if (isBulk && urls.length > 5) {
    return res.status(400).json({
      error: "Maximum 5 URLs per bulk request on serverless. Run in batches.",
    });
  }
  ...
  try {
    ...
    // Source - https://stackoverflow.com/a/67117619
    // Posted by Andrew Mititi, modified by community. See post 'Timeline' for change history
